...
[![Performance](https://img.shields.io/badge/performance-0.04ms-brightgreen)](https://github.com/ljlm0402/fynk#-성능-벤치마크)
...
> **자동 중복 제거 및 정규화 캐시를 제공하는 가장 빠른 HTTP 클라이언트**
...
function UserProfile({ userId }: { userId: number }) {
  ...
  return (
    <div>
      <h1>{data?.name}</h1>
      <button onClick={() => mutate({ name: '새 이름' })}>
        이름 업데이트
      </button>
    </div>
  );
...
*벤치마크: 동일 엔드포인트에 대한 10개 동시 동일 요청*
